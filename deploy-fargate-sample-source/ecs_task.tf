  ...
  container_definitions = jsonencode([
    {
      ...
      secrets = [
        { name = "ENCRYPT_API_TOKEN", valueFrom = "${data.aws_secretsmanager_secret_version.secretsmanager_secret.arn}:ENCRYPT_API_TOKEN::" },
        { name = "ENCRYPT_ID", valueFrom = "${data.aws_secretsmanager_secret_version.secretsmanager_secret.arn}:ENCRYPT_ID::" }
      ]

      environment: [
        { name = "API_BASE", value = "https://launchpad-assistant-api.launchpad.tech.gov.sg/services/openai/" }
      ],
