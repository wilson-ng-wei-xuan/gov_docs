...
# from app.models.flan.flan_model import FlanGenerationModel
...
# model_flan_generation = FlanGenerationModel(dynamodb, TABLE_MOONSHOT_LLM)

sns_client = boto3.client(service_name='sns', region_name=AWS_REGION_NAME)


class FlanGenerationType(BaseModel):
    prompt: str
...
@router.post("/flan_chat")
async def flan_chat(payload: FlanChatType):
    """
    Calls Flan with the prompt and returns the response
    """
    ...
    flan_response = await llm_util.flan_chat(messages, temperature, max_tokens)
    print(flan_response)
    ...
    if flan_response:
        ...
        response_text = flan_response

        # Store records of flan usage by storing user, prompt, model and response
        # model_flan_generation.put_generation_response(id, prompt, model, temperature, response_text, llm_util.encrypt_identity(caller))

        # Return flan response
        return {
            "id": id,
            "messages": messages,
        ...
        raise HTTPException(
            status_code=500,
            detail=f"flan error for messages: {messages}",
        )
...
#     gpt_response = model_flan_generation.get_item(response_id=response_id)
...
#             model_flan_generation.mark_emailed(response_id=response_id)
...
#     upvote_response = model_flan_generation.upvote_response(response_id=response_id)
...
#     downvote_response = model_flan_generation.downvote_response(response_id=response_id)
