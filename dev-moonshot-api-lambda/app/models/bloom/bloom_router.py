...
# from app.models.bloom.bloom_model import BloomGenerationModel
...
# model_bloom_generation = BloomGenerationModel(dynamodb, TABLE_MOONSHOT_LLM)

sns_client = boto3.client(service_name='sns', region_name=AWS_REGION_NAME)


class BloomGenerationType(BaseModel):
    prompt: str
...
@router.post("/bloom_generation")
async def bloom_generation(payload: BloomGenerationType):
    """
    Calls Bloom with the prompt and returns the response
    """
    ...
    bloom_response = await llm_util.bloom_completion(prompt, temperature, max_tokens)
    ...
    if bloom_response:
        ...
        response_text = bloom_response.replace('<|endoftext|>','').strip()

        # Store records of bloom usage by storing user, prompt, model and response
        # model_bloom_generation.put_generation_response(id, prompt, model, temperature, response_text, llm_util.encrypt_identity(caller))
        ...
        raise HTTPException(
            status_code=500,
            detail=f"bloom error for prompt: {prompt}",
        )

@router.post("/bloom_chat")
async def bloom_chat(payload: BloomChatType):
    """
    Calls Bloom with the prompt and returns the response
    """
    ...
    bloom_response = await llm_util.bloom_chat(messages, temperature, max_tokens)
    print(bloom_response)
    ...
    if bloom_response:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Bloom error for messages: {messages}",
        )
...
#     gpt_response = model_bloom_generation.get_item(response_id=response_id)
...
#             model_bloom_generation.mark_emailed(response_id=response_id)
...
#     upvote_response = model_bloom_generation.upvote_response(response_id=response_id)
...
#     downvote_response = model_bloom_generation.downvote_response(response_id=response_id)
