...
from app.config import TABLE_MOONSHOT_LLM, PALM_CHAT_MODEL_DEFAULT, DATETIME_TZ_FORMAT
...
class PalmChatModel(DynamodbModel):
    ...
    def put_chat_response(self, response_id, conversation_id, conversation_title, messages, model, temperature, response, caller, tokens_used, pinned):
        """
        Store PALM message/model/response and additional params into DynamoDB
        """
        if not response_id:
...
if __name__ == "__main__":
    ...
    model = PalmChatModel(dynamodb, TABLE_MOONSHOT_LLM)

    prompt = "Say this is a test"
    messages = [{"user":prompt}]

    response = llm_util.palm_chat(messages=messages, model=PALM_CHAT_MODEL_DEFAULT)
