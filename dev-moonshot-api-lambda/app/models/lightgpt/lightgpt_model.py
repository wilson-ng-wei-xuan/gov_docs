...
from app.config import TABLE_MOONSHOT_LLM, DATETIME_MS_FORMAT
...
class LightGPTGenerationModel(DynamodbModel):
    ...
    def put_generation_response(self, response_id, prompt, model, temperature, response, caller):
        """
        Store LightGPT prompt/model/response and additional params into DynamoDB
        """
        if not response_id:
...
if __name__ == "__main__":
    ...
    model = LightGPTGenerationModel(dynamodb, TABLE_MOONSHOT_LLM)

    prompt = "Say this is a test"

    response = llm_util.completion(prompt=prompt)
