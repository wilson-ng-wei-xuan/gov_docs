...
def poetry_lock(session: nox_poetry.Session):
    ...
    Args:
        session (nox_poetry.Session): Nox session
    """
    print(f"Updating poetry.lock files for agents-api")
...
def build(session: nox_poetry.Session):
    ...
    Args:
        session (nox_poetry.Session): Nox session
    """
    print(f"Building Agents API library")
...
def publish(session: nox_poetry.Session):
    """
    Builds a full distributable version of Atlas to the
    ...
    print(f"Publishing Atlas library to {package_registry}")
...
def tag(session: nox_poetry.Session):
    ...
    """
    atlas_tag: str = session.poetry.poetry.config._config['version']
    message: str = f"Version v{atlas_tag}"

    print(f"Tagging AIBots with {atlas_tag} {message}")
    session.run(
        "git",
        "tag",
        "-a",
...
def untag(session: nox_poetry.Session):
    ...
    message: str = f"Version v{atlas_tag}"
    if session.posargs:
        repo: str = session.posargs[0]
    else:
        repo: str = "origin"

    print(f"Untagging AIBots with {atlas_tag} {message}")
    session.run(
        "git",
        "tag",
        "-d",
