...
from ..lambda_function import HTMLParser


class TestHTMLParser:

    def test_html_parser_result_success(self, request) -> None:
        """tests"""
        parse_config: Dict[str, Any] = request.getfixturevalue("html_parse_config")
        mock_aws_infra: boto3.Session = request.getfixturevalue("mock_aws_infra")
        html_knowledge_base: KnowledgeBase = request.getfixturevalue("html_knowledge_base")
        ...
        message.knowledge_bases.append(html_knowledge_base)
        ...
        parser = HTMLParser(message, sqs, environ)
    ...
    def test_html_parser_next_success(self, request, chunker) -> None:
