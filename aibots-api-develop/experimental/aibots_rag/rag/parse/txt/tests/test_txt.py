...
from ..lambda_function import TxtParser


class TestPDFParser:

    def test_txt_parser_result_success(self, request) -> None:
        """tests"""
        parse_config: Dict[str, Any] = request.getfixturevalue("txt_parse_config")
        mock_aws_infra: boto3.Session = request.getfixturevalue("mock_aws_infra")
        txt_knowledge_base: KnowledgeBase = request.getfixturevalue("txt_knowledge_base")
        ...
        message.knowledge_bases.append(txt_knowledge_base)
        ...
        parser = TxtParser(message, sqs, environ)
    ...
    def test_txt_parser_next_success(self, request, chunker) -> None:
