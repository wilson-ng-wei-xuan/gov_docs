...
from ..lambda_function import PDFParser


class TestPDFParser:

    def test_pdf_parser_result_success(self, request) -> None:
        """tests"""
        parse_config: Dict[str, Any] = request.getfixturevalue("pdf_parse_config")
        mock_aws_infra: boto3.Session = request.getfixturevalue("mock_aws_infra")
        pdf_knowledge_base: KnowledgeBase = request.getfixturevalue("pdf_knowledge_base")
        ...
        message.knowledge_bases.append(pdf_knowledge_base)
        ...
        parser = PDFParser(message, sqs, environ)
    ...
    def test_pdf_parser_next_success(self, request, chunker) -> None:
