...
from ..lambda_function import PptxParser


class TestPptxParser:

    def test_pptx_parser_result_success(self, request) -> None:
        """tests"""
        parse_config: Dict[str, Any] = request.getfixturevalue("pptx_parse_config")
        mock_aws_infra: boto3.Session = request.getfixturevalue("mock_aws_infra")
        pptx_knowledge_base: KnowledgeBase = request.getfixturevalue("pptx_knowledge_base")
        ...
        message.knowledge_bases.append(pptx_knowledge_base)
        ...
        parser = PptxParser(message, sqs, environ)
    ...
    def test_pptx_parser_next_success(self, request, chunker) -> None:
