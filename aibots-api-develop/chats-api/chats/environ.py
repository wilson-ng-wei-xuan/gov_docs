...
from atlas.schemas import Email
from pydantic import AliasChoices, AnyUrl, Field, field_validator

from chats.constants import DEFAULT_APP_MESSAGES
...
__all__ = ("AIBotsChatEnviron", "get_environ")


class AIBotsChatEnviron(
    AppEnviron,
    AWSEnvVars,
    DBEnvVars,
    ...
    Attributes:
        ...
        cloak (ServiceEnvVars | None): Cloak service env vars
        vectordb_opensearch (ServiceEnvVars | None): OpenSearch Vector DB env vars
    """  # noqa: E501
    ...
    nous_api: Annotated[
        ServiceEnvVars | None,
        Field(validation_alias=AliasChoices("NOUS_API", "NOUS-API")),
    ] = None
    ...
    cloak: ServiceEnvVars | None = None
...
environ: AIBotsChatEnviron | None = None


@lru_cache
def get_environ() -> AIBotsChatEnviron:
    """
    Convenience function to retrieve a cached version
    of the AppEnviron setting

    Returns:
        AIBotsChatEnviron: Application environment variables
    """
    global environ
    if not environ:
        environ = AIBotsChatEnviron()
    return environ
