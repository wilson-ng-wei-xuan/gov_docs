...
################################################################################
  ...
  nacl_ids = local.nacl_aibots_db

  nacl_rules = [
################################################################################
# allow inbound from aibots app
################################################################################
    ...
    { # allow inbound from app to mongo
      action      = "allow"
      direction   = "inbound"
      from_port   = 27017
      to_port     = 27017
      cidr_block  = data.aws_subnet.aibots_ez["${local.subnet_prefix}-a-${terraform.workspace}ezapp-aibots"].cidr_block
    },
    { # allow inbound from app to mongo
      action      = "allow"
      direction   = "inbound"
      from_port   = 27017
      to_port     = 27017
      cidr_block  = data.aws_subnet.aibots_ez["${local.subnet_prefix}-b-${terraform.workspace}ezapp-aibots"].cidr_block
    },
    { # allow inbound from app to opensearch
      action      = "allow"
      direction   = "inbound"
      from_port   = 443
      to_port     = 443
