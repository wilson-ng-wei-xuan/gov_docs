...
locals {
  ...
  ${upper( replace(var.project_code, "-", "_") )}_${upper( replace(var.project_desc, "-", "_") )}__PUB_URL = "${lower(var.protocol)}://${var.pub.host == null ? "not_set" : var.pte.host}$${local.PUB_URL}"
...
data "aws_ssm_parameter" "${var.project_code}-${var.project_desc}" {
  name  = "$${local.para_store_prefix}-$${terraform.workspace}${var.zone}${var.tier}-${var.project_code}-${var.project_desc}-aoai"
}
