...
module "lambda" {
  ...
  environment_variables = {
    ...
    # # EXTRAS
    ############################################################################
    AIBOTS__PTE_URL                       = local.AIBOTS_MAIN_API__PTE_URL
    AIBOTS__PUB_URL                       = local.AIBOTS_MAIN_API__PUB_URL
    AIBOTS__PARAM                         = local.AIBOTS__PARAM
    GOVTEXT__BUCKET                       = local.SHAREDSVC_SCHEDULER__BUCKET
    GOVTEXT__PARAM                        = local.AIBOTS_GOVTEXT__PARAM
    GOVTEXT__SQS                          = module.sqs[each.value.name].sqs_queue.name
          ...
          Statement = [
            {
              ...
              Resource = [
                ...
                data.aws_ssm_parameter.aibots-govtext.arn,
                data.aws_ssm_parameter.aibots-main-api.arn,
