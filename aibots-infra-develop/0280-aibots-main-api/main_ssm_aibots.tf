...
resource "aws_ssm_parameter" "aibots" {
  lifecycle {
    ignore_changes = [
      value,
    ]
  ...
  name        = "${local.para_store_name}" #because proj_desc is blank, thus there is already a -
  description = "The AIBots endpoint for ${var.project_code}"
  type        = "SecureString"
  value       = jsonencode(
    {
