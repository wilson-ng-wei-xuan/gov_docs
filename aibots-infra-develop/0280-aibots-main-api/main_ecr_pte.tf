...
resource "aws_ecr_repository" "project_pte" {
  count = var.pte.host != null ? 1 : 0

  name                 = "${local.ecr_name}-${var.pte.name}"
...
resource "aws_ecr_lifecycle_policy" "project_pte" {
  count = var.pte.host != null ? 1 : 0

  repository = aws_ecr_repository.project_pte[0].name
