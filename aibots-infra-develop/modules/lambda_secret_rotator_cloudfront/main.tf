locals {
  ...
  function_name = substr( "${local.lambda_prefix}-${local.tags.Environment}${local.tags.Zone}-${local.tags.Project-Code}-secret-rotator-cloudfront", 0, 64)
}
        ...
        policy = jsonencode(
          {
            ...
            Statement = [
              ...
              },
              {
                ...
                Resource = "arn:aws:ssm:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:parameter/${local.para_store_prefix}-${terraform.workspace}${local.tags.Zone}${local.tags.Tier}-${local.tags.Project-Code}-${local.tags.Project-Desc}-publickey"
              },
              {
                Effect = "Allow",
                Action = [
