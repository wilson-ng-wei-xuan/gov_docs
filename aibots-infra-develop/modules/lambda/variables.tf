...
variable "image_uri" {
  ...
  default     = null
  description = <<EOT
    (Required) ECR image URI containing the function deployment package.
    Refer to: https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/lambda_function
  EOT
}
...
variable "ignore_changes" {
  ...
  default     = false
  description = <<EOT
    (Optional) if you want to ignore_changes in lifecycle
    Terraform does not allow passing ignore_changes as variable. This variable only toggles which resource to use.
    simple_lambda or simple_lambda_ignore_changes is use.
  EOT
}
...
#   description = <<EOT
#     This is the trusted relationships, default is the "lambda.amazonaws.com".
#     But in some case you will need to allow AWS services like "secretsmanager.amazonaws.com"
#     Refer to: https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/lambda_function
#   EOT
# }