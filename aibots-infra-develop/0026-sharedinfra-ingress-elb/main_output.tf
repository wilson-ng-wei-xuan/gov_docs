...
###########################################################
data "aws_lb" "${var.zone}${var.tier}${local.nlb_prefix}_pub" {
  tags = {
...
data "aws_lb_listener" "${var.zone}${var.tier}${local.nlb_prefix}_pub_443" {
  load_balancer_arn = data.aws_lb.${var.zone}${var.tier}${local.alb_prefix}_pub.arn
  port              = 443
}

data "aws_security_group" "${var.zone}${var.tier}${local.nlb_prefix}_pub" {
  filter {
    name     = "tag:Terraform"
    values = ["true"]
  }
