module "project_pte" {
  ...
  #               # data.aws_secretsmanager_secret.aibots_project_pte.arn,
  #               resource.aws_secretsmanager_secret.project[0].arn,
  ...
  container_definitions    = [ {
    ...
    #   # # ADD YOUR VARIABLES HERE: project_pte
    #   { name = "PROJECT__PUB_URL", value = "https://${local.PUB_URL}" },
    #   { name = "PROJECT_API__PTE_URL", value = local.AIBOTS_MAIN_API__PTE_URL },
    #   { name = "PROJECT_API__PUB_URL", value = local.AIBOTS_MAIN_API__PUB_URL },
    #   { name = "PROJECT__BUCKET", value = local.AIBOTS_PROJECT__BUCKET },
    #   # { name = "PROJECT__SECRET",value = local.AIBOTS__SECRET },
