...
data "template_file" "env_var" {
  ...
  vars        = { # these are the variables to render
    ...
    # PROJECT_RAG_EMBED_NOUS__SQS           = local.AIBOTS_RAG_EMBED_NOUS__SQS
    ############################################################################
    # # CHUNK use by PARSE
    ############################################################################
    # PROJECT_RAG_CHUNK_ZIP__SQS            = local.AIBOTS_RAG_CHUNK_ZIP__SQS
    # PROJECT_RAG_CHUNK_TEXT_SEMANTIC__SQS  = local.AIBOTS_RAG_CHUNK_TEXT_SEMANTIC__SQS
    # PROJECT_RAG_CHUNK_TEXT_FIXED__SQS     = local.AIBOTS_RAG_CHUNK_TEXT_FIXED__SQS
    # PROJECT_RAG_CHUNK_TEXT_ANCHORS__SQS   = local.AIBOTS_RAG_CHUNK_TEXT_ANCHORS__SQS
    # PROJECT_RAG_CHUNK_DATAFRAME__SQS      = local.AIBOTS_RAG_CHUNK_DATAFRAME__SQS
    ############################################################################
    # # PARSE use by SOURCE
    ############################################################################
    PROJECT_RAG_PARSE_ZIP__SQS              = local.AIBOTS_RAG_PARSE_ZIP__SQS
    PROJECT_RAG_PARSE_DOCX__SQS             = local.AIBOTS_RAG_PARSE_DOCX__SQS
    PROJECT_RAG_PARSE_XLSX__SQS             = local.AIBOTS_RAG_PARSE_XLSX__SQS
    PROJECT_RAG_PARSE_PPTX__SQS             = local.AIBOTS_RAG_PARSE_PPTX__SQS
    PROJECT_RAG_PARSE_CSV__SQS              = local.AIBOTS_RAG_PARSE_CSV__SQS
    PROJECT_RAG_PARSE_TXT__SQS              = local.AIBOTS_RAG_PARSE_TXT__SQS
    PROJECT_RAG_PARSE_PDF__SQS              = local.AIBOTS_RAG_PARSE_PDF__SQS
    ############################################################################
    # # EXTSTACK use by SOURCE
    ############################################################################
    PROJECT_RAG_EXTSTACK_GOVTEXT__SQS       = local.AIBOTS_RAG_EXTSTACK_GOVTEXT__SQS
    ############################################################################
    # # SOURCE can self use by SOURCE
    ############################################################################
    PROJECT_RAG_SOURCE_ZIP__SQS             = module.sqs["zip"].sqs_queue.url
    PROJECT_RAG_SOURCE_URL__SQS             = module.sqs["url"].sqs_queue.url
    PROJECT_RAG_SOURCE_S3__SQS              = module.sqs["s3"].sqs_queue.url
    ############################################################################
    # # EXTRAS
    ############################################################################
...
data "template_file" "policy" {
  ...
  vars        = { # these are the variables to render
    ...
    # PROJECT_RAG_EMBED_NOUS__SQS_ARN       = data.aws_sqs_queue.aibots_rag-embed_nous.arn
    ############################################################################
    # # CHUNK use by PARSE
    ############################################################################
    # PROJECT_RAG_CHUNK_ZIP__SQS_ARN            = data.aws_sqs_queue.aibots_rag-chunk_zip.arn
    # PROJECT_RAG_CHUNK_TEXT_SEMANTIC__SQS_ARN  = data.aws_sqs_queue.aibots_rag-chunk_text_semantic.arn
    # PROJECT_RAG_CHUNK_TEXT_FIXED__SQS_ARN     = data.aws_sqs_queue.aibots_rag-chunk_text_fixed.arn
    # PROJECT_RAG_CHUNK_TEXT_ANCHORS__SQS_ARN   = data.aws_sqs_queue.aibots_rag-chunk_text_anchors.arn
    # PROJECT_RAG_CHUNK_TEXT_DATAFRAME__SQS_ARN = data.aws_sqs_queue.aibots_rag-chunk_dataframe.arn
    ############################################################################
    # # PARSE use by SOURCE
    ############################################################################
    PROJECT_RAG_PARSE_ZIP__SQS_ARN            = data.aws_sqs_queue.aibots_rag-parse_zip.arn
    PROJECT_RAG_PARSE_DOCX__SQS_ARN           = data.aws_sqs_queue.aibots_rag-parse_docx.arn
    PROJECT_RAG_PARSE_XLSX__SQS_ARN           = data.aws_sqs_queue.aibots_rag-parse_xlsx.arn
    PROJECT_RAG_PARSE_PPTX__SQS_ARN           = data.aws_sqs_queue.aibots_rag-parse_pptx.arn
    PROJECT_RAG_PARSE_CSV__SQS_ARN            = data.aws_sqs_queue.aibots_rag-parse_csv.arn
    PROJECT_RAG_PARSE_TXT__SQS_ARN            = data.aws_sqs_queue.aibots_rag-parse_txt.arn
    PROJECT_RAG_PARSE_PDF__SQS_ARN            = data.aws_sqs_queue.aibots_rag-parse_pdf.arn
    ############################################################################
    # # EXTSTACK use by SOURCE
    ############################################################################
    PROJECT_RAG_EXTSTACK_GOVTEXT__SQS_ARN     = data.aws_sqs_queue.aibots_rag-extstack_govtext.arn
    ############################################################################
    # # SOURCE can self use by SOURCE, e.g. URL after wget will call s3 to start the pipe.
    ############################################################################
    PROJECT_RAG_SOURCE_ZIP__SQS_ARN           = module.sqs["zip"].sqs_queue.arn
    PROJECT_RAG_SOURCE_URL__SQS_ARN           = module.sqs["url"].sqs_queue.arn
    PROJECT_RAG_SOURCE_S3__SQS_ARN            = module.sqs["s3"].sqs_queue.arn
    ############################################################################
    # # EXTRAS
    ############################################################################
...
module "lambda" {
  ...
  memory_size = 256
  security_group_ids = each.value.name == "url" ? [] : [ data.aws_security_group.aibots_ez["${local.secgrp_prefix}-${terraform.workspace}${var.zone}${var.tier}-${var.project_code}"].id ]
  ...
  subnet_ids = each.value.name == "url" ? [] : [ data.aws_subnet.aibots_ez["${local.subnet_prefix}-a-${terraform.workspace}${var.zone}${var.tier}-${var.project_code}"].id,
