...
resource "aws_ssm_parameter" "cloak" {
  lifecycle {
    ignore_changes = [
      value,
    ]
  ...
  name        = "${local.para_store_name}cloak" #because proj_desc is blank, thus there is already a -
  description = "The cloak endpoint for ${var.project_code}"
  type        = "SecureString"
  value       = jsonencode(
    {
      endpoint = "change via console"
      id = "change via console"
