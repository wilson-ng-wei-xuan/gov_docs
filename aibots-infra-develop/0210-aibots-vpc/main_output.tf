...
resource "local_file" "outputdata-ext_api" {
  filename        = "${path.module}/../data/data.${local.path}.ext_api.tf"
...
################################################################################
data "aws_ssm_parameter" "${var.project_code}-cloak" {
  name  = "$${local.para_store_prefix}-$${terraform.workspace}${var.zone}-${var.project_code}-cloak"
}

data "aws_ssm_parameter" "${var.project_code}-llmstack" {
  name  = "$${local.para_store_prefix}-$${terraform.workspace}${var.zone}-${var.project_code}-llmstack"
}

data "aws_ssm_parameter" "${var.project_code}-govtext" {
  name  = "$${local.para_store_prefix}-$${terraform.workspace}${var.zone}-${var.project_code}-govtext"
}


# USE AS ENVIRONMENT VARIABLE
locals {
  ${upper( replace(var.project_code, "-", "_") )}_CLOAK__PARAM = "$${data.aws_ssm_parameter.${var.project_code}-cloak.name}"
  ${upper( replace(var.project_code, "-", "_") )}_LLMSTACK__PARAM = "$${data.aws_ssm_parameter.${var.project_code}-llmstack.name}"
  ${upper( replace(var.project_code, "-", "_") )}_GOVTEXT__PARAM = "$${data.aws_ssm_parameter.${var.project_code}-govtext.name}"
}
EOT
}
