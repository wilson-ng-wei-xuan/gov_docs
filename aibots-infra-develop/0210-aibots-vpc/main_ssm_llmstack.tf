...
resource "aws_ssm_parameter" "llmstack" {
  lifecycle {
    ignore_changes = [
      value,
    ]
  ...
  name        = "${local.para_store_name}llmstack" #because proj_desc is blank, thus there is already a -
  description = "The llmstack endpoint for ${var.project_code}"
  type        = "SecureString"
  value       = jsonencode(
    {
